#include "include/BinaryResource.h"
#include <Log.h>
#include <algorithm>
#include <fstream>
namespace Corona {
    static bool endsWith(const std::string &s,const std::string &suf){ if(s.size()<suf.size()) return false; return std::equal(suf.rbegin(),suf.rend(),s.rbegin()); }
    bool BinaryResourceLoader::supports(const ResourceId &id) const { for(const auto &e: exts_) if(endsWith(id.path,e)) return true; return false; }
