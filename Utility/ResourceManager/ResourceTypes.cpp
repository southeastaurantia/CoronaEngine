...
namespace Corona
{
    ...
    {
        auto a = uid ? uid : ComputeUid(type, path);
        auto b = o.uid ? o.uid : ComputeUid(o.type, o.path);
        return a == b;
    }
    bool ResourceId::operator<(const ResourceId &o) const noexcept
    {
    ...
    std::uint64_t SubResourceId::Compute(std::uint64_t parent, SubResourceKind kind, std::uint64_t localNumeric, std::string_view localString)
    {
        ...
        const std::uint64_t FNV_PRIME = 1099511628211ull;
        auto h = FNV_OFFSET;
        auto mix64 = [&](std::uint64_t v) { for(int i=0;i<8;++i){ unsigned char b= static_cast<unsigned char>(v & 0xFF); h^=b; h*=FNV_PRIME; v >>=8; } };
        mix64(parent);
        mix64(static_cast<std::uint64_t>(kind));
        mix64(localNumeric);
        for (unsigned char c : localString)
        {
            h ^= c;
            h *= FNV_PRIME;
        }
    ...
    SubResourceId SubResourceId::FromIndex(const ResourceId &parent, SubResourceKind kind, std::uint64_t index)
    {
        SubResourceId s;
    ...
    SubResourceId SubResourceId::FromKey(const ResourceId &parent, SubResourceKind kind, std::string_view key)
    {
        SubResourceId s;
        s.parentUid = parent.uid ? parent.uid : ResourceId::ComputeUid(parent.type, parent.path);
        s.kind = kind;
    ...
    SubResourceId SubResourceId::Compose(std::uint64_t parentUid, SubResourceKind kind, std::uint64_t localNumeric, std::string_view localString)
    {
        SubResourceId s;
