...
namespace Corona
{
    ...
    std::uint64_t SubResourceId::Compute(std::uint64_t parent,
                                         ...
                                         std::string_view localString)
    {
        ...
        auto mix64 = [&](std::uint64_t v) {
            for (int i = 0; i < 8; ++i)
            {
                unsigned char b = static_cast<unsigned char>(v & 0xFF);
                h ^= b;
                h *= FNV_PRIME;
                v >>= 8;
            }
    ...
    SubResourceId SubResourceId::Compose(std::uint64_t parentUid, SubResourceKind kind,
