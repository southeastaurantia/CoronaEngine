...
|--------|----------------------|--------------------|--------------------|
| `core` | `src/core/public` | `CoronaEngineAPI.h`, `Engine.h`, `EngineKernel.h`, `SystemRegistry.h`, `SystemHubs.h`, `components/*`, `events/*`, `testing/MockServices.h` | Runtime entry points, engine embedding, system registration, component/event definitions |
...
| `thread` | (all headers currently under `src/thread/public`) | `SafeCommandQueue.h`, `SafeDataCache.h`, `EventBus.h` | Used by systems & engine |
...
include/
  corona/
    ...
    systems/                             # optional direct system access
      ...
      DisplaySystem.h
      RenderingSystem.h
    threading/                           # formerly src/thread/public
      SafeCommandQueue.h
      SafeDataCache.h
      EventBus.h
...
```
Additional notes:
- `EngineKernel.h`, `SystemHubs.h`, and other purely internal headers will migrate to `src/core/`.
...
2. **Core module cleanup:**
   ...
   - Relocate internal-only headers (`EngineKernel.h`, `SystemHubs.h`, etc.) into `src/core/` and adjust includes.
...
- [ ] Draft a mapping table for `src/core/public` headers to their new home (mark which remain public vs become internal).
- [ ] Identify headers in `src/thread/public`, `src/utils/public`, and `src/script/public` that must stay public and assign future locations.
