...
static inline void APISafeDecRef(PyObject* obj, const char* tag) {
    if (!obj) {
        std::cout << "[Hotfix][API] SafeDecRef skip null: " << tag << std::endl;
        return;
    }
    ...
    if (rc64 <= 0 || rc64 > 1000000000LL) {
        std::cout << "[Hotfix][API] SafeDecRef suspicious refcnt, skip: " << tag << std::endl;
        return;
    }
    Py_DECREF(obj);
}
...
PythonAPI::PythonAPI() {
    ...
    if (const char* env = std::getenv("CORONA_PY_LEAKSAFE")) {
        ...
        if (v == "1" || v == "true" || v == "on" || v == "yes")
            leakSafeMainReload_ = true;
        else if (v == "0" || v == "false" || v == "off" || v == "no")
            leakSafeMainReload_ = false;
    }
...
bool PythonAPI::performHotReload() {
    ...
    if (!new_mod) {
        ...
        if (!from_sys_modules && !leakSafeMainReload_) {
            APISafeDecRef(mod, "main.mod(owned, reload fail)");
        }
        if (reload_func && !leakSafeMainReload_) Py_XDECREF(reload_func);
        if (importlib && !leakSafeMainReload_) Py_XDECREF(importlib);
        return false;
    ...
    if (!leakSafeMainReload_) {
        if (new_mod != mod) {
            APISafeDecRef(new_mod, "main.new_mod post-swap");
        }
    } else {
