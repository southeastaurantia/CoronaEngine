...
void BindAll(nanobind::module_& m) {
    ...
    nanobind::class_<CoronaEngineAPI::Camera>(m, "Camera")
        .def(nanobind::init<>())
        .def("__init__", [](CoronaEngineAPI::Camera* self,
                             const std::array<float, 3>& position,
                             const std::array<float, 3>& forward,
                             const std::array<float, 3>& world_up,
                             float fov) {
            new (self) CoronaEngineAPI::Camera(vec3_from(position), vec3_from(forward), vec3_from(world_up), fov);
        }, nanobind::arg("position"), nanobind::arg("forward"), nanobind::arg("world_up"), nanobind::arg("fov"))
        .def("set_surface", [](const CoronaEngineAPI::Camera& self, std::uintptr_t surface) {
            self.set_surface(reinterpret_cast<void*>(surface));
        }, nanobind::arg("surface"))
        .def("set_position", [](const CoronaEngineAPI::Camera& self, const std::array<float, 3>& v) {
            self.set_position(vec3_from(v));
        }, nanobind::arg("position"))
        .def("get_position", [](const CoronaEngineAPI::Camera& self) -> std::array<float, 3> {
            auto v = self.get_position();
            return {{v.x, v.y, v.z}};
        })
        .def("set_forward", [](const CoronaEngineAPI::Camera& self, const std::array<float, 3>& v) {
            self.set_forward(vec3_from(v));
        }, nanobind::arg("forward"))
        .def("get_forward", [](const CoronaEngineAPI::Camera& self) -> std::array<float, 3> {
            auto v = self.get_forward();
            return {{v.x, v.y, v.z}};
        })
        .def("set_world_up", [](const CoronaEngineAPI::Camera& self, const std::array<float, 3>& v) {
            self.set_world_up(vec3_from(v));
        }, nanobind::arg("world_up"))
        ...
        .def("move", [](const CoronaEngineAPI::Camera& self, const std::array<float, 3>& v) {
            self.move(vec3_from(v));
        }, nanobind::arg("delta"))
        .def("rotate", [](const CoronaEngineAPI::Camera& self, const std::array<float, 3>& v) {
            self.rotate(vec3_from(v));
        }, nanobind::arg("euler"))
        .def("look_at", [](const CoronaEngineAPI::Camera& self, const std::array<float, 3>& pos, const std::array<float, 3>& forward) {
            self.look_at(vec3_from(pos), vec3_from(forward));
        }, nanobind::arg("position"), nanobind::arg("forward"));
    ...
    nanobind::class_<CoronaEngineAPI::Scene>(m, "Scene")
        ...
        .def("set_sun_direction", [](const CoronaEngineAPI::Scene& self, const std::array<float, 3>& dir) {
            self.set_sun_direction(vec3_from(dir));
        }, nanobind::arg("direction"))
