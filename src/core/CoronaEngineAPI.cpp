...
CoronaEngineAPI::Scene::Scene(void* surface, bool /*lightField*/)
    ...
    if (surface) {
        ...
        Corona::Engine::instance().events<Corona::SceneEvents::DisplaySurfaceUpdated>().publish("scene.surface", Corona::SceneEvents::DisplaySurfaceUpdated{sid, surface});
    }
}
...
CoronaEngineAPI::Scene::~Scene() {
    ...
    const std::uint64_t sid = static_cast<std::uint64_t>(static_cast<std::uint32_t>(sceneID));
    Corona::Engine::instance().events<Corona::SceneEvents::Removed>().publish("scene.removed", Corona::SceneEvents::Removed{sid});
...
void CoronaEngineAPI::Scene::setCamera(const ktm::fvec3& position, const ktm::fvec3& forward, const ktm::fvec3& worldUp, float fov) const {
    ...
    // 事件广播：摄像机参数副本
    const std::uint64_t sid = static_cast<std::uint64_t>(static_cast<std::uint32_t>(sceneID));
    Corona::Engine::instance().events<Corona::SceneEvents::CameraUpdated>().publish("scene.camera", Corona::SceneEvents::CameraUpdated{sid, fov, position, forward, worldUp});
}
...
void CoronaEngineAPI::Scene::setSunDirection(ktm::fvec3 direction) const {
    ...
    // 事件广播：太阳光方向副本
    const std::uint64_t sid = static_cast<std::uint64_t>(static_cast<std::uint32_t>(sceneID));
    Corona::Engine::instance().events<Corona::SceneEvents::SunUpdated>().publish("scene.sun", Corona::SceneEvents::SunUpdated{sid, direction});
}
...
CoronaEngineAPI::Actor::Actor(const std::string& path)
    ...
    Corona::Engine::instance().events<Corona::ActorEvents::Spawned>().publish("actor.spawn", Corona::ActorEvents::Spawned{aid, path});
}
...
CoronaEngineAPI::Actor::~Actor() {
    ...
    // 广播：Actor 移除
    Corona::Engine::instance().events<Corona::ActorEvents::Removed>().publish("actor.removed", Corona::ActorEvents::Removed{aid});
...
void CoronaEngineAPI::Actor::move(ktm::fvec3 pos) const {
    const std::uint64_t aid = static_cast<std::uint64_t>(static_cast<std::uint32_t>(actorID));
    Corona::Engine::instance().events<Corona::ActorEvents::TransformUpdated>().publish("actor.transform", Corona::ActorEvents::TransformUpdated{aid, pos, {}, {}});
}

void CoronaEngineAPI::Actor::rotate(ktm::fvec3 euler) const {
    const std::uint64_t aid = static_cast<std::uint64_t>(static_cast<std::uint32_t>(actorID));
    Corona::Engine::instance().events<Corona::ActorEvents::TransformUpdated>().publish("actor.transform", Corona::ActorEvents::TransformUpdated{aid, {}, euler, {}});
}

void CoronaEngineAPI::Actor::scale(ktm::fvec3 size) const {
    const std::uint64_t aid = static_cast<std::uint64_t>(static_cast<std::uint32_t>(actorID));
    Corona::Engine::instance().events<Corona::ActorEvents::TransformUpdated>().publish("actor.transform", Corona::ActorEvents::TransformUpdated{aid, {}, {}, size});
}
