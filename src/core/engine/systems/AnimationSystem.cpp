...
void AnimationSystem::onTick()
{
    ...
    animCache.safe_loop_foreach(state_cache_keys_, [&](const std::shared_ptr<AnimationState>& st) {
        if (!st || !st->model) {
            return;
        }
    ...
    modelCache.safe_loop_foreach(model_cache_keys_, [&](uint64_t id, const std::shared_ptr<Model>& m) {
        if (!m) {
            return;
        }
...
void AnimationSystem::updatePhysics(Model &m)
{
    ...
    auto &modelCache = Engine::Instance().Cache<Model>();
    modelCache.safe_loop_foreach(other_model_cache_keys_, [&](const std::shared_ptr<Model>& otherModel) {
        if (!otherModel) {
            return;
        }
